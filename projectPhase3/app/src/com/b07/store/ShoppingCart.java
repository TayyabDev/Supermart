 ...
 */
  public void restoreShoppingCart() throws SQLException, InvalidInputException, InvalidIdException,
      ItemNotFoundException, InvalidQuantityException, DatabaseInsertException {
    ...
    if (quantity >= 0) {
      ...
      for (Item currentItem : DatabaseSelectHelper.getAllItems()) {
        if (currentItem.getName().equals(item.getName())) {
          ...
          // insert the item into the account line record if user has account
          if (hasAccount) {
            DatabaseInsertHelper.insertAccountLine(customerAccountId, item.getId(), quantity);
          }
    ...
    if (quantity >= 0) {
      ...
      for (Item currentItem : this.getItems()) {
        if (currentItem.getName().equals(item.getName())) {
          ...
          if (hasAccount) {
            // insert the removal of item into the database if user has account
