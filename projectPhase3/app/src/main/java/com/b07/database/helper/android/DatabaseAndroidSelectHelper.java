...
public class DatabaseAndroidSelectHelper extends DatabaseDriverAndroid {
  ...
  public Inventory getInventoryHelper() {
    ...
    while (c.moveToNext()) {
      ...
      try {
        inventory.updateItemMap(getItemHelper(c.getInt(c.getColumnIndex("ITEMID"))),
  ...
  /**
   * Populate the sale object's item map with the items stored in the database corresponding to it's
   * sale ID.
   *
   * @param saleId the Id of the sale
  ...
  public void getItemizedSaleByIdHelper(int saleId, Sale sale) throws InvalidIdException {
    ...
    while (c.moveToNext()) {
      ...
      itemMap.put(getItemHelper(c.getInt(c.getColumnIndex("ITEMID"))),
          c.getInt(c.getColumnIndex("QUANTITY")));
      System.out.println(
          c.getInt(c.getColumnIndex("ITEMID")) + " " + c.getInt(c.getColumnIndex("QUANTITY")));
    }
    c.close();
  ...
  /**
   * Given a list of sales (saleslog), populate each sale in the saleslog's itemmap given their
   * itemized sales.
   *
