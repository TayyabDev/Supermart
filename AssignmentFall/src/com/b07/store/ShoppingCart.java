...
public class ShoppingCart {
   ...
   * @throws CustomerNoAccountException if activity_customer tries to restore cart without an
   *         account
   */
    ...
    if (quantity >= 0) {
      ...
      for (Item currentItem : this.getItems()) {
        if (currentItem.getId() == item.getId()) {
          ...
          // multiply the price of the item * quantity
          BigDecimal priceBefore =
              BigDecimal.valueOf(cart.get(currentItem)).multiply(item.getPrice());
  ...
  /**
   * Determine if customer has inactive account.
   * @return if customer has inactive account
   * @throws InvalidIdException if customer has invalid id
   ...
   * @return if customer has active account
   * @throws InvalidIdException if customer has invalid id
   * @throws SQLException if sql fault.
   */
  ...
  private int getCustomerActiveAccountId()
      throws InvalidIdException, CustomerNoAccountException, SQLException {
  ...
  private int getCustomerInactiveAccountId()
      throws InvalidIdException, CustomerNoAccountException, SQLException {
    // get customers active accounts
    List<Integer> inactiveAccountIds =
        DatabaseSelectHelper.getUserInactiveAccounts(customer.getId());
  ...
  /**
   * Update the customer's account with the items in his shopping cart.
   * @return if update was successful.
   * @throws SQLException  if sql fault
   * @throws DatabaseInsertException if insert error
   * @throws InvalidInputException if input invalid
   * @throws InvalidQuantityException if quantity of any item is invalid
   * @throws InvalidIdException if customer id invalid
   * @throws InvalidRoleException if customer not given role yet
   */
  public boolean updateAccount() throws SQLException, DatabaseInsertException,
      InvalidInputException, InvalidQuantityException, InvalidIdException, InvalidRoleException {
    ...
    try {
      ...
      for (Item item : this.getItems()) {
        // insert account line
        DatabaseInsertHelper.insertAccountLine(customerAccountId, item.getId(),
  ...
  /**
   * Take a shopping cart with associated activity_customer then calculate the total after tax and
   * submit to database. If the item are enough to be purchased, update the table and clear the
   * shopping cart.
   * 
